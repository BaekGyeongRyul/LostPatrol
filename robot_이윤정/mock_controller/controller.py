...
# 순찰 전용으로 더 느린 속도를 따로 둠(2026.08.27). 2026.08.30 "천천히"
# 요청으로 80->40까지 낮췄다가, 그러면 모터가 정지 마찰(스틱션)을 못
# 이겨서 명령은 가는데 바퀴가 아예 안 도는 문제 발견 — 확실히 움직이는
# 선에서 다시 60으로 올림.
PATROL_SPEED = 60

# 바깥쪽 센서(많이 벗어남)가 반응했을 때 회전 쪽 바퀴 속도(2026.08.30
# 추가, 아래 _patrol_steer 참고). 살살 커브 트는 것보다, 안쪽 두 센서로
# 빨리 복귀하는 게 우선이라는 피드백으로 0으로 낮춤 — 회전 쪽 바퀴는
# 완전히 멈추고 반대쪽만 굴려서, 그 바퀴를 축으로 빠르게 방향을 튼다
# (완전한 제자리 회전은 아님 — 반대쪽 바퀴가 계속 전진하니 차체는 계속
# 앞으로 나아가면서 정렬됨).
PATROL_TURN_SPEED = 0

# 라인을 완전히 놓쳤을 때(4개 센서 다 흰색) 제자리에서 좌우로 번갈아
# 돌면서 라인을 다시 찾는 속도. 20은 모터가 아예 안 돌아서(스틱션) 35로 올림.
SEARCH_SPIN_SPEED = 35
