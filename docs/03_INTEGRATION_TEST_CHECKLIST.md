...
| 5 | Web에서 **PATROL START** 클릭 | 라인트레이싱 자동순찰이 시작됨 | 이윤정 | 🔶 라인트레이싱 센서 4채널 단독 동작은 실물로 확인 완료(2026.08.26, 트리머 감도 캘리브레이션 후 흰바탕/검은선에 따라 정확히 0/1 반전 확인). 웹 PATROL START 트리거로 실제 자동순찰 동작 통합 테스트는 아직 |
...
| 9 | 자동순찰 중 초음파 센서로 장애물 감지 | 설정된 거리 이내 장애물 발견 시 로봇이 **안전 정지** (회피 아님) | 이윤정 | 🔶 초음파 센서 단독 동작은 실물로 확인(2026.08.26, `_measure_distance_cm()`이 거리에 따라 값 정확히 변화). 순찰 중 정지 로직 통합 테스트는 5번(순찰)이 아직 통합 테스트 전이라 못함 |
| 10 | 카메라 앞에 umbrella / bottle / backpack 노출 | YOLO가 탐지하고(2~3회 연속 검출 확인 후) `lost_items`에 새 행 INSERT (`status='new'`) | 조은수 | ☑ 파이프라인은 실물로 확인 완료(2026.08.26, 이윤정: 실물 Razbot 카메라로 backpack 촬영→연속2회→INSERT 성공 id=4). **단, 모델 분류 정확도는 미흡** — 스테인리스 텀블러는 conf 0.05 이하로도 미검출, 투명 페트병은 umbrella로 오분류(conf 0.72~0.75, id=6). 조은수에게 재학습 피드백 전달 필요(파이프라인 버그 아님, 학습 데이터 문제로 판단) |
| 11 | 위 탐지 시 촬영된 이미지 | `lost-item-photos` 버킷에 업로드되고, 그 공개 URL이 `lost_items.image_url`에 저장되어 **웹 화면에 실제 이미지로 표시**됨 | 조은수 | ☑ 2026.08.26 실물로 확인 완료 — Storage 업로드+공개 URL 저장+웹 표시까지 전부 실제 데이터(id=4, id=6)로 검증됨 |
...
- (2026.08.26) 테스트 중 생성된 더미 행 `lost_items.id=6`(item_type이 umbrella로 잘못 등록됨, 실제로는 bottle)은 anon key로 삭제 불가 — 백경률이 Dashboard에서 삭제 예정.
